...
class Mdl_Clients extends MY_Model {
	...
	public function __construct() {
		...
		(SELECT SUM(invoice_total) FROM mcb_invoice_amounts WHERE invoice_id IN (SELECT invoice_id FROM mcb_invoices WHERE client_id = join_client_id)) AS client_total_invoice,
		IFNULL((SELECT SUM(payment_amount) FROM mcb_payments JOIN mcb_invoices ON mcb_invoices.invoice_id = mcb_payments.invoice_id WHERE mcb_invoices.client_id = mcb_clients.client_id), 0.00) AS client_total_payment,
	...
	public function get($params = NULL) {
		...
		if (is_array($clients)) {
			...
			if ($this->mdl_mcb_data->version > '0.8.2') {
