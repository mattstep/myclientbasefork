		...
		<div class="content toggle">
			...
			<form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>">
				...
				<dl>
					...
					<dd><input type="text" name="invoice_number" value="<?php echo $this->mdl_invoice_search->invoice_number; ?>" /></dd>
				</dl>

				<dl>
					<dt><label><?php echo $this->lang->line('from_date'); ?>: </label></dt>
					<dd><input type="text" class="datepicker" name="from_date" value="<?php echo $this->mdl_invoice_search->from_date; ?>" /></dd>
				</dl>

				<dl>
					<dt><label><?php echo $this->lang->line('to_date'); ?>: </label></dt>
					<dd><input type="text" class="datepicker" name="to_date" value="<?php echo $this->mdl_invoice_search->to_date; ?>" /></dd>
				</dl>
					...
					<dd>
						<select name="client_id[]" id="client_id" multiple size="10" style="width: 400px;">
							<?php foreach ($clients as $client) { ?>
								<option value="<?php echo $client->client_id; ?>" <?php if ($this->mdl_invoice_search->client_id == $client->client_id) { ?>selected="selected"<?php } ?>><?php echo $client->client_name; ?></option>
							<?php } ?>
						</select>
					</dd>
				</dl>
					...
					<dd>
						<select name="invoice_status_id[]" id="invoice_status_id" multiple size="5" style="width: 400px;">
							<?php foreach ($invoice_statuses as $invoice_status) { ?>
								<option value="<?php echo $invoice_status->invoice_status_id; ?>" <?php if ($this->mdl_invoice_search->invoice_status_id == $invoice_status->invoice_status_id) { ?>selected="selected"<?php } ?>><?php echo $invoice_status->invoice_status; ?></option>
							<?php } ?>
						</select>
					</dd>
				</dl>
				...
				<dl>
					...
					<dd>
						...
						<input type="text" name="amount" value="<?php echo $this->mdl_invoice_search->amount; ?>" />
					</dd>
				</dl>

				<dl>
					<dt><label><?php echo $this->lang->line('tags'); ?>: </label></dt>
					<dd><input id="tags" type="text" name="tags" value="<?php echo $this->mdl_invoice_search->tags; ?>" /></dd>
				</dl>
