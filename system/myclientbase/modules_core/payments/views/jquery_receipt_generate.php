	...
	$(function() {
		...
		function generate_receipt() {
			...
			if (output_type != 'email') {
				...
				var email_url = '<?php echo site_url('payments/send_email/form/invoice_id'); ?>' + '/' + invoice_id + '/receipt_template/' + receipt_template;
		...
		<tr>
			...
			<td>
				<select name="receipt_template" id="receipt_template">
					...
					<option <?php if ($template == $this->mdl_mcb_data->default_receipt_template) { ?>selected="selected"<?php } ?>><?php echo $template; ?></option>
					<?php } ?>
				</select>
			</td>
		</tr>
