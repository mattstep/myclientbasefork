...
class Dashboard extends Admin_Controller {
	...
	function index() {
		...
		if ($this->mdl_mcb_data->dashboard_show_open_invoices == 'TRUE') {

			$data['open_invoices'] = $this->mdl_invoices->get_open();

		}

		if ($this->mdl_mcb_data->dashboard_show_pending_invoices == 'TRUE') {

			$data['pending_invoices'] = $this->mdl_invoices->get_pending();

		}

		if ($this->mdl_mcb_data->dashboard_show_closed_invoices == 'TRUE') {

			$data['closed_invoices'] = $this->mdl_invoices->get_closed();

		}

		if ($this->mdl_mcb_data->dashboard_show_overdue_invoices == 'TRUE') {

			$data['overdue_invoices'] = $this->mdl_invoices->get_overdue();
