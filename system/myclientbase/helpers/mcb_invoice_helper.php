...
function invoice_to_address($invoice) {
	...
	/* Client address */
	return $invoice->client->client_address;
...
function invoice_to_address_2($invoice) {
	...
	/* Client address 2 */
	return $invoice->client->client_address_2;
...
function invoice_to_city($invoice) {
	...
	/* Client city */
	return $invoice->client->client_city;
...
function invoice_to_city_state_zip($invoice) {
	...
	if ($invoice->client->client_city and $invoice->client->client_state) {

		return $invoice->client->client_city . ', ' . $invoice->client->client_state . ' ' . $invoice->client->client_zip;
...
function invoice_to_country($invoice) {
	...
	/* Client country */
	return $invoice->client->client_country;
...
function invoice_to_full_address($invoice) {
	...
	if ($invoice->client->client_address) {
		...
		$address .= $invoice->client->client_address . '<br />';

		if ($invoice->client->client_address_2) {

			$address .= $invoice->client->client_address_2 . '<br />';
...
function invoice_to_phone_number($invoice) {
	...
	/* Client phone number */
	return $invoice->client->client_phone_number;
...
function invoice_to_state($invoice) {
	...
	/* Client state */
	return $invoice->client->client_state;
...
function invoice_to_zip($invoice) {
	...
	/* Client zip code */
	return $invoice->client->client_zip;
...
function invoice_client_tax_id($invoice) {
	...
	return $invoice->client->client_tax_id;
...
function invoice_date_entered($invoice) {
	...
	return date($CI->mdl_mcb_data->default_date_format, $invoice->invoice_date_entered);
...
function invoice_due_date($invoice) {
	...
	return date($CI->mdl_mcb_data->default_date_format, $invoice->invoice_due_date);
...
function invoice_logo() {
	...
	if ($CI->mdl_mcb_data->include_logo_on_invoice == 'TRUE' AND $CI->mdl_mcb_data->invoice_logo) {

		if ($CI->uri->segment(2) == 'generate_pdf' or $CI->uri->segment(2) == 'send_email') {

			return '<img src="' . getcwd() . '/uploads/invoice_logos/' . $CI->mdl_mcb_data->invoice_logo . '" />';

		}

		else {

			return '<img src="' . base_url() . 'uploads/invoice_logos/' . $CI->mdl_mcb_data->invoice_logo . '" />';
